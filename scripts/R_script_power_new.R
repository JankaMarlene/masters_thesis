...
wilcox.test(mean_beta1_power~cluster_2, data = df_corr_central1_filtered, 
            exact = FALSE, 
            correct = FALSE, 
            conf.int = FALSE)
...
# in order to get the z value
result <- coin::wilcox_test(data = df_corr_central1_filtered,mean_beta1_power~cluster_2, comparisons = list(c('c1','c2')), alternative = 'two.sided')

# get effsize
df_corr_central1_filtered%>%
  ungroup()%>%
  wilcox_effsize(mean_beta1_power~cluster_2)
# exploratory: beta 2
df_corr_central2_filtered%>%
  mutate(cluster_2 = fct_recode(cluster_2,
                            "c1" = "c1",
...
# in order to get the z value
result <- coin::wilcox_test(data = df_corr_central2_filtered,mean_beta2_power~cluster_2, comparisons = list(c('c1','c2')), alternative = 'two.sided')

# get effsize
df_corr_central2_filtered%>%
  ungroup()%>%
  wilcox_effsize(mean_beta2_power~cluster_2)
...
df_corr_central1_filtered%>%
  group_by(cluster_2)%>%
  ggplot(aes(x = mean_beta1_power,y = tmt_a_time, color = cluster_2))+
  geom_point()
...
df_corr_central1_filtered%>%
  group_by(cluster_2)%>%
  ggplot(aes(x = mean_beta1_power,y = tmt_diff,color = cluster_2))+
  geom_point()

cor.test(df_corr_central1_filtered$mean_beta1_power,df_corr_central1_filtered$tmt_diff, method = 'spearman', exact = FALSE)

# beta 2
df_corr_central2_filtered%>%
  group_by(cluster_2)%>%
  ggplot(aes(x = mean_beta2_power,y = tmt_a_time, color = cluster_2))+
  geom_point()
...
df_corr_central2_filtered%>%
  group_by(cluster_2)%>%
  ggplot(aes(x = mean_beta2_power,y = tmt_diff,color = cluster_2))+
  geom_point()

cor.test(df_corr_central2_filtered$mean_beta2_power,df_corr_central2_filtered$tmt_diff, method = 'spearman', exact = FALSE)
